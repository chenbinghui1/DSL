...
# Thie script is stolen from STAC, and with a few changes.
...
def prepare_coco_data(seed=1, percent=10.0, version=2017):
  ...
  def _save_anno(name, images, annotations):
    ...
    with open(
        '{folder}/{save_name}.json'.format(
            save_name=name,  folder='semi_supervised'),
        'w') as f:
  ...
  li = list(range(len(image_list)))
  random.shuffle(li)
  labeled_ind = li[0:labeled_tot]
  labeled_id = []
  labeled_images = []
  unlabeled_images = []
  ...
  shutil.copyfile(os.path.join(DATA_DIR, 'annotations', 'instances_train{}.json'.format(version)), os.path.join(path, 'instances_train{}.json'.format(version)))
  shutil.copyfile(os.path.join(DATA_DIR, 'annotations', 'instances_val{}.json'.format(version)), os.path.join(path, 'instances_val{}.json'.format(version)))


if __name__ == '__main__':

  parser = argparse.ArgumentParser()
  parser.add_argument('--percent', type=float, default=10, help="percent of labeled data")
  parser.add_argument('--version', type=int, default=2017, help="2017")
  parser.add_argument('--root', type=str, default='/gruntdata2/tcguo/coco/', help="Original COCO dir path")
